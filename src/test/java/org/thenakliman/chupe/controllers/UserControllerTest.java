...
public class UserControllerTest {
    ...
    public void shouldReturnEmptyUser() throws Exception{
        ...
        List<User> result = objectMapper.readValue(mvcResult.getResponse().getContentAsString(), List.class);
        assertEquals(result, exp_users);
    }

    @Test
    public void shouldReturnUser() throws Exception {
        User modelUser1 = new User("user1_fistname", "user1_lastname", "user1_username", "user1_email", "user1_password", true);
        User modelUser2 = new User("user2_fistname", "user2_lastname", "user2_username", "user2_email", "user2_password", false);
        List<User> modelUsers= new ArrayList<User>();
        ...
        mvcResult = mockMvc.perform(MockMvcRequestBuilders
            .get("/api/v1/users")).andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
        /* NOTE(thenakliman): Use jackson to convert response to object and then compare the objects, currently
         * only length is being matched.
         */
        List<User> result = objectMapper.readValue(mvcResult.getResponse().getContentAsString(), new ArrayList<User>().getClass());
    ...
    public void shouldReturnInternalServerErrorIfThereIsAnException() throws Exception {
        ...
        BDDMockito.given(userService.getAllUsers()).willAnswer(invocation -> {throw new Exception();});
